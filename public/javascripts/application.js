...
var TT = {
  ...
  setTips : function() {
    ...
    TT.popupHeader = $('div#trip-list tr:first').clone();
...
var showOnly = function(letter, dest) {
  ...
  //trip-list
  $('div#trip-list tr.trip').each(function(i, e) {
...
$(document).ready(function() {
  ...
  $('div#destination-list table td').each(function(i, e) {
    ...
    if ($('#week_calendar').size() > 0) {$('a#week-view').css({'color':'black', 'text-decoration':'underline'}) }
    if ($('table#wip').size() > 0) {$('a#wip-view').css({'color':'black', 'text-decoration':'underline'})}
  ...
  $('table#wip table').each(function() {
    $(this).find('tr').each(function(i) {
      if ((i + 3) % 4 == 0) {
        $(this).find('td').css("border-bottom", "1px solid black")
      }
    })
  })
